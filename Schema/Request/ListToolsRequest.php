...
final readonly class ListToolsRequest extends PaginatedRequest implements ClientRequest
{
    ...
    {
        Assert::that($data)->hasOffset('id', 'ListToolsRequest wire data missing "id".');
        $id = $data['id'];
        Assert::that($id)->isArrayKey('ListToolsRequest wire "id" must be int or string, {type} given.');

        $params = new PaginatedRequestParams();

        if (\array_key_exists('params', $data)) {
            Assert::that($data['params'])
                ->isArray('ListToolsRequest wire "params" must be an object, {type} given.')
                ->isMap('ListToolsRequest wire "params" must be a string-keyed object.')
            ;
