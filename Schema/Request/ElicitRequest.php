...
final readonly class ElicitRequest extends JsonRpcRequest implements ServerRequest
{
    ...
    {
        Assert::that($data)->hasOffset('id', 'ElicitRequest wire data missing "id".');
        $id = $data['id'];
        Assert::that($id)->isArrayKey('ElicitRequest wire "id" must be int or string, {type} given.');

        Assert::that($data)->hasOffset('params', 'ElicitRequest wire data missing "params".');
        Assert::that($data['params'])
            ->isArray('ElicitRequest wire "params" must be an object, {type} given.')
            ->isMap('ElicitRequest wire "params" must be a string-keyed object.')
        ;

        $mode = $data['params']['mode'] ?? ElicitRequestFormParams::MODE;
        Assert::that($mode)->isString('ElicitRequest wire params "mode" must be a string, {type} given.');
