...
final readonly class GetTaskPayloadRequestParams extends RequestParams
{
    ...
    {
        Assert::that($data)->hasOffset('taskId', 'GetTaskPayloadRequestParams wire data missing "taskId".');
        $taskId = $data['taskId'];
        Assert::that($taskId)->isString('GetTaskPayloadRequestParams wire "taskId" must be a string, {type} given.');
