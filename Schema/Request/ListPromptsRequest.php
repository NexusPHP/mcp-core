...
final readonly class ListPromptsRequest extends PaginatedRequest implements ClientRequest
{
    ...
    {
        Assert::that($data)->hasOffset('id', 'ListPromptsRequest wire data missing "id".');
        $id = $data['id'];
        Assert::that($id)->isArrayKey('ListPromptsRequest wire "id" must be int or string, {type} given.');

        $params = new PaginatedRequestParams();

        if (\array_key_exists('params', $data)) {
            Assert::that($data['params'])
                ->isArray('ListPromptsRequest wire "params" must be an object, {type} given.')
                ->isMap('ListPromptsRequest wire "params" must be a string-keyed object.')
            ;
