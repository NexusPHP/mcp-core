...
final readonly class GetTaskRequest extends JsonRpcRequest implements ClientRequest, ServerRequest
{
    ...
    {
        Assert::that($data)->hasOffset('id', 'GetTaskRequest data missing "id".');
        $id = $data['id'];
        Assert::that($id)->isArrayKey('GetTaskRequest "id" must be int or string, {type} given.');

        Assert::that($data)->hasOffset('params', 'GetTaskRequest data missing "params".');
        Assert::that($data['params'])
            ->isArray('GetTaskRequest "params" must be an object, {type} given.')
            ->isMap('GetTaskRequest "params" must be a string-keyed object.')
        ;
